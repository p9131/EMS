    ...
    public static ArrayList<Employee> getEmployees() throws IOException, InterruptedException, ParseException{
        ...
        for(int i = 0; i < size;i++){
            ...
            try{
                ...
                ea.add(new Employee(jo.getString("name")
                        ,gender
                        ,rank
...
//    });
    ...
    /*not tested yet*/public static ArrayList<Employee> selectEmployees(String cond,Object value) throws IOException, InterruptedException, ParseException{
        ArrayList<Employee> ea=new ArrayList<Employee>();
        JSONArray eag=GET("employee-db?"+cond+"="+value.toString());
        JSONObject jo =new JSONObject();
        int size=eag.length();
        System.out.println("size of the json object is "+size);
        ...
        for(int i = 0; i < size;i++){
            ...
            try{
                System.out.println(jo.getString("gender"));
                ea.add(new Employee(jo.getString("name"),(jo.get("gender")).toString(),jo.getNumber("rank").intValue()/*,jo.getString("pass")*/
                        ,jo.getString("jobDescription"),jo.getString("email"),jo.getString("typeOfEmployment"),new SimpleDateFormat("dd/MM/yyyy").parse(jo.getString("birthdate"))
    ...
    private static ArrayList<Task> taskSchemetoArr(JSONArray ja) throws ParseException{
        ...
        for (int i = 0; i < size; i++) {
            ...
            ta.add(new Task(jo.getString("name")
                    ...
                    ,new SimpleDateFormat("dd-MM-yyyy").parse(jo.getString("deadline"))
            ));
        }
        return ta;
    }
...
//          data.put("tasks",taskScheme(eo.getTasks()));//json array of tasks
        ...
        upByCondition("employee-db","username",setter.toArray(new String[0]), eo.getUsername(), Val.toArray(new Object[0]));
        setter.clear();
        Val.clear();
    }
...
//        raiseRequests: {type: }
    public static void addTeam(Team to) throws IOException, InterruptedException{
        ...
        upByCondition("team-db","username",setter.toArray(new String[0]), to.getManagerID(), Val.toArray(new Object[0]));
        setter.clear();
        Val.clear();
    }
    //delete teams
    ...
    public static void updateSpecficOfTeam(Team to) throws IOException, InterruptedException{
        ...
        addVacationsTeam(to,true);
        addRaiseRequestsTeam(to, true);
        upByCondition("team-db","managerID",setter.toArray(new String[0]),to.getManagerID(),Val.toArray(new Object[0]));
        setter.clear();
        Val.clear();
    }
    ...
    public static void updateSpecficOfEvent(Event eo) throws IOException, InterruptedException{
        ...
        upByCondition("event-db","name",setter.toArray(new String[0]),eo.getName(),Val.toArray(new Object[0]));
        setter.clear();
        Val.clear();
    }
    ...
    private static void upByCondition(String Path,String condition, String[] setter, Object val1,Object[] val2) throws IOException, InterruptedException{
        Map<Object, Object> data = new HashMap<>();
